...
import { Users, Plus, Search, Edit2, Trash2, Save, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const StudentsPage = () => {
  ...
  const handleAdd = async () => {
    setSaving(true);
    try { await createStudent(addForm); setAddForm(emptyForm); setShowAdd(false); load(); }
    catch { alert('Error adding student'); }
    finally { setSaving(false); }
  };

  const handleEdit = (s) => { setEditId(s.id); setEditForm({ ...s }); };
  const handleCancel = () => setEditId(null);

  const handleSave = async (id) => {
    setSaving(true);
    try { await updateStudent(id, editForm); setEditId(null); load(); }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
        ...
        {showAdd && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="glass-card p-6 border border-primary/30 grid grid-cols-1 md:grid-cols-3 gap-4">
              <h3 className="text-sm font-bold text-primary uppercase tracking-widest md:col-span-3">New Student</h3>
              {[['Full Name', 'name'], ['Email', 'email'], ['Section', 'section']].map(([label, key]) => (
      ...
      <section className="glass-card overflow-hidden">
        {loading ? (
          <div className="text-center py-20 opacity-50">Loading students...</div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-gray-300">
                ...
                <tbody>
                  {paginated.map((s, i) => (
                    <>
                      <tr key={s.id} className="border-b border-border hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-[10px] bg-white/5 text-textMuted px-2 py-0.5 rounded uppercase font-bold tracking-wider">{s.section}</span>
                        </td>
                        <td className="px-6 py-4 flex items-center justify-end gap-3">
                          <button onClick={() => editId === s.id ? handleCancel() : handleEdit(s)} className={editId === s.id ? 'text-gray-400 hover:text-white' : 'text-primary hover:text-primary/70'}>
                            {editId === s.id ? <X size={18} /> : <Edit2 size={18} />}
                          </button>
                          <button onClick={() => handleDelete(s.id)} className="text-red-400 hover:text-red-300"><Trash2 size={18} /></button>
                        </td>
                      </tr>
                      <AnimatePresence>
                        {editId === s.id && (
                            ...
                            <td colSpan="5" className="p-0">
                              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                                <div className="bg-white/5 border-b border-border px-6 py-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                                  ...
                                  <div className="flex gap-3 md:col-span-3">
                                    <button onClick={() => handleSave(s.id)} disabled={saving} className="btn-primary px-6 py-2.5 text-sm">
              ...
              {filtered.length === 0 && <div className="text-center py-10 opacity-50">No students found.</div>}
            </div>
